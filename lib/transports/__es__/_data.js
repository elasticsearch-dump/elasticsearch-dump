...
class Data {
  async _getDataPit (searchBody, callback) {
    const isOpensearch = this.ESDistribution.startsWith("opensearch")

    if (!this.lastSearchAfter) {
      // Initialize PIT if enabled
      if (this.parent.options.pit) {
        const uri = isOpensearch
          ? `${this.base.url}/*/_search/point_in_time?keep_alive=${this.parent.options.pitKeepAlive || '5m'}`
        ...
        try {
          ...
          this.pitId = isOpensearch ? parsed.pit_id : parsed.id
        } catch (err) {
          return callback(err, [])
        }
      }
