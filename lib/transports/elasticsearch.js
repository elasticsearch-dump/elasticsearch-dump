...
class elasticsearch {
  ...
  renderTemplate (id, params) {
    ...
    let renderTemplateRequestBody = {'params': params}

    var renderTemplateRequest = {
      uri: uri,
      method: 'GET',
    ...
    return new Promise((resolve, reject) => {
      this.baseRequest(renderTemplateRequest, (err, success) => {
        if (!err && success.statusCode==200) {
          let render = jsonParser.parse(success.body).template_output
          resolve(render)
        }
        else {
          reject(err)
  ...
  setData (data, limit, offset, callback) {
    ...
    data.forEach(elem => {
      ...
      if (this.parent.options.handleVersion) {
        ...
        if (this.parent.options.handleVersion.versionType) {
          actionMeta.index.version_type = this.parent.options.handleVersion.versionType
        }
      }
