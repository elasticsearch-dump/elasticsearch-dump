...
elasticsearch.prototype.set = function(data, limit, offset, callback){
  ...
  if (self.parent.options.bulk === true){
    ...
    data.forEach(function(elem){
      payload.body += "{ \"index\" : { ";
      payload.body += "\"_index\" : \"" + elem._index + "\", ";
      payload.body += "\"_type\" : \""  + elem._type  + "\", ";
      payload.body += "\"_id\" : \""    + elem._id    + "\"";
      payload.body += " } }\n";
    ...
    request.put(payload, function(err, response, body){
      try{
        ...
        if(r.ok == true){ writes += data.length; }
      }catch(e){ }
      self.reindex(function(err){
    ...
    data.forEach(function(elem){
      ...
      if(self.parent.options.head === true){ thisUrl += elem._index + "/"; }
