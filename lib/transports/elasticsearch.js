...
// return (error, arr) where arr is an array of objects
  get (limit, offset, callback) {
    const self = this
    const type = self.parent.options.type
  ...
  getData (limit, offset, callback) {
    let searchRequest, self, uri
    self = this
    const searchBody = self.searchBody
    ...
    if (self.lastScrollId !== null) {
      ...
      const paramsObj = self.parent.options.params
      ...
      self.baseRequest(searchRequest, function requestResonse (err, response) {
        ...
        const body = jsonParser.parse(response.body, self.parent)
...
// return (error, writes)
      ...
      self.baseRequest(esRequest, function (err, response) { // ensure the index exists
        ...
        for (let index in data) {
          ...
          async.eachSeries(sortedMappings, function (set, done) {
            const url = self.base.url + '/' + encodeURIComponent(set.key) + '/_mapping'
            const payload = {
              url: url,
              method: 'PUT',
  ...
  setAnalyzer (data, limit, offset, callback) {
    const self = this
    const updateAnalyzer = function (err, response) {
      if (err) { return callback(err) }
      ...
      for (let index in data) {
        ...
        for (let key in settings) { // iterate through settings
          ...
          const url = self.base.url + '/_settings'
          started++
          count++
  ...
  setSettings (data, limit, offset, callback) {
    ...
    async.forEachOf(data, function (index, name, cb) {
      let settings = _.omit(index, self.settingsExclude)
  ...
  setData (data, limit, offset, callback) {
    ...
    const self = this
    let extraFields = ['routing', 'parent', 'timestamp', 'ttl']
    let writes = 0

    extraFields = _.chain(this.parent.options.parseExtraFields)
      .split(',')
      .concat(extraFields)
      .flatten()
      .uniq()
    ...
    const thisUrl = self.base.url + '/_bulk'
    ...
    self.baseRequest(payload, function (err, response) {
      ...
      try {
        const r = jsonParser.parse(response.body, self.parent)
  ...
  del (elem, callback) {
    const self = this
    const thisUrl = self.base.host + '/' + encodeURIComponent(elem._index) + '/' + encodeURIComponent(elem._type) + '/' + encodeURIComponent(elem._id)
