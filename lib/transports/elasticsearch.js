...
elasticsearch.prototype.version = function (prefix, callback) {
  ...
  request.get(self.base.host, function (err, response) {
    ...
    if (response.version) {
      ...
      self.parent.emit('log', 'discovered elasticsearch ' + prefix + ' major version: ' + self.ESversion)
    } else {
      self.ESversion = 5
      self.parent.emit('log', 'cannot discover elasticsearch ' + prefix + ' major version, assuming: ' + self.ESversion)
    }
