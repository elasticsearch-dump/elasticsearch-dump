...
elasticsearch.prototype.getMapping = function (limit, offset, callback){
    ...
    if(self.gotMapping === true){
        ...
        request.get(url, function(err, response){
            ...
            var payload = [];
            if(!err){ response = payload.push(JSON.parse(response.body)); }
    ...
    var updateAnalyzer = function(err, response){
        ...
        for(var index in data){
            ...
            for(var key in settings){ // interate through settings
                ...
                request.post({url: closeUrl, timeout: self.parent.options.timeout}, function(err, response, body) {
                    if(!err) {
                        var bodyError = jsonParser.parse(response.body).error;
                        if(bodyError){ err = bodyError; }
                        var payload = {
                        ...
                        request.put(payload, function(err, response){ // upload the analysis settings
                            ...
                            if(started === 0){
                                ...
                                request.post({url: openUrl, timeout: self.parent.options.timeout}, function(err, response) {
                                    if(!err){
                                        var bodyError = jsonParser.parse(response.body).error;
                                        if(bodyError){ err = bodyError; }
                                    }
...
elasticsearch.prototype.setData = function(data, limit, offset, callback){
    ...
    if (self.parent.options.bulk === true){
        ...
        var bulkMode = self.parent.options['bulk-mode'] || 'index';
        data.forEach(function(elem, index, theArray) {
            var actionMeta = {};
            var meta = actionMeta[bulkMode] = {};
            if (!useOutputIndexName) { // Not setting _index should use the one defined in URL.
                meta._index = elem._index;
            }
            meta._type = elem._type;
            meta._id = elem._id;
            extraFields.forEach(function(field) {
                if (elem.fields[field]) {
                    meta[field] = elem.fields[field];
                } else if (elem[field]) {
                    meta[field] = elem[field];
                }

                if (elem.fields['_' + field]) {
                    meta[field] = elem.fields['_' + field];
                } else if (elem['_' + field]) {
                    meta[field] = elem['_' + field];
                }
            });
            var val = JSON.stringify(actionMeta) + "\n";
            if (bulkMode !== 'delete') {
                var actionData = (bulkMode === 'update')
                    ? { doc: elem._source, doc_as_upsert: true }
                    : elem._source;
                val += JSON.stringify(actionData) + "\n";
            }
            theArray[index] = val;
        });
