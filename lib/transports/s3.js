...
class s3 extends base {
  ...
  async setupGet (offset) {
    ...
    if (this.parent.options.s3Compress) {
      ...
      // Add error handling for zlib decompression errors
      this._throughStream.on('error', (err) => {
        if (err.code === 'Z_BUF_ERROR' || err.code === 'Z_DATA_ERROR') {
          this.parent.emit('error', new Error(`Decompression failed: ${err.message}. The file may be corrupted or not properly compressed.`))
        } else {
