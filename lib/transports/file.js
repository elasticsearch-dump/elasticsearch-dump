...
/**
 * from https://stackoverflow.com/a/5973162/423943
 * @param s
 * @returns {string}
 */
function getFileExtension (s) {
  ...
  const extension = s.substr(dot, s.length)
  return extension
}
    ...
    if (!self.stream) {
      if (self.file === '$') {
        ...
        if (!self.parent.options.overwrite && fs.existsSync(self.file)) {
          ...
          // do nothing !
        } else if (getFileExtension(self.file) === '.gz') {
          const compress = zlib.createGzip()
          const output = fs.createWriteStream(self.file)
