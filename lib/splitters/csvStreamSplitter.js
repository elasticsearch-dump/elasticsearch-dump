...
class csvStreamSplitter extends StreamSplitter {
  ...
  _outStreamCreate (partitionNum) {
    ...
    if (this.compress) {
      _throughStream = zlib.createGzip({ level: this._ctx.parent.options.compressionLevel })
    }
    const fileStream = fs.createWriteStream(StreamSplitter.generateFilePath(this.rootFilePath, partitionNum, this.compress))
    _csvStream.pipe(_throughStream).pipe(fileStream)
    this.streamList.push(fileStream)
    return _csvStream
  }
