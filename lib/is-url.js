...
const fromCsvUrl = (uri) => {
  csvUrlRegex.lastIndex = 0
  const matches = csvUrlRegex.exec(uri)

  if (!matches) {
    throw new Error(`Invalid CSV Url: ${uri}`)
  }

  const [protocol, url] = _.tail(matches)
...
const isCsvUrl = (url) => {
  csvUrlRegex.lastIndex = 0
  const matches = csvUrlRegex.exec(url)
  return !!matches
}
