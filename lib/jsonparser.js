...
const stringify = (json, configs) => {
  try {
    return _.get(configs, 'options.support-big-int')
      ? LosslessJSON.stringify(json, _replacer.call(this, _getBigIntFields(configs)))
      : JSON.stringify(json)
  } catch (e) {
    if (/^Invalid string length$/ig.test(e.message)) {
      return JSON.stringify({
        message: `The maximum size allowed for a string: ${buffer.constants.MAX_STRING_LENGTH / (1024 * 1024)} MB`
      })
    }
    throw e
  }
